...
| [telefonwerbung](telefonwerbung/INDEX.md) | Wann ein Werbeanruf zulässig ist, was die mutmaßliche Einwilligung nicht trägt, die Anrufmaschinen-Frage bei KI, Rufnummernanzeige, Bußgelder; dazu die Gegenrichtung: wer hinter einer Nummer steckt und wie man Rufnummernmissbrauch meldet | Vor jeder Akquise am Telefon, vor jedem ausgehenden Anruf einer KI, bevor ein Werbemittel einen Anruf ankündigt, und wenn eine unbekannte Nummer Max angerufen hat |
...
| [shared-db-verbindungen](shared-db-verbindungen/INDEX.md) | Jahresarchiv aller Anmeldungen an `shared-db`: Ablage, Suche nach Adresse oder Fehlanmeldung, Laufkontrolle, `psql` nur mit `-U postgres`, Zähler ab Reset | Bei „wer hat sich an der Datenbank angemeldet", nach jedem Sicherheitsvorfall, und vor jedem `psql` auf `shared-db` |
...
| [gmail-mcp](gmail-mcp/INDEX.md) | Bibel: Gmail über MCP. Der echte Anmeldebefehl, zwei Pakete mit fast gleichem Namen, ein Token-Ordner, den Codex beim Abmelden leert | Wenn ein Gmail-Kanal „Not authenticated" meldet, und bevor irgendwo `gmail_logout` gerufen wird |
| [skillspector](skillspector/INDEX.md) | Skills und MCP-Server vor der Installation prüfen: Aufrufe, Urteilsstufen, der Fehlalarm AE1, der Preis der LLM-Stufe über `claude_cli` | Vor jeder Installation eines fremden Skills oder MCP-Servers, und bei „ist dieser Skill sicher“ |
