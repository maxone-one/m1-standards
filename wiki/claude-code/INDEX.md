...
- [Mehrere Claude-Abos, eines je Projekt](mehrere-abos-pro-projekt.md) — wie das Kundenabo nur im Kundenprojekt zieht und der eigene Zugang ueberall sonst: `CLAUDE_CONFIG_DIR` je VS-Code-Profil, warum die Terminal-Loesungen aus dem Netz hier ausscheiden, und **warum der naheliegende Weg ueber ein Token in den Projekt-Settings nicht traegt** (gemessen, Credential-Variablen sind dort gefiltert). Enthaelt die Einrichtungsstrecke fuer den NUC, Schritt fuer Schritt, am 09.09.2026 auf dem NUC durchlaufen und belegt (zwei Konten, vier Sitzungen gleichzeitig)
- [Mehrere Claude-Abos, die Belege](mehrere-abos-pro-projekt-belege.md) — die Messungen dahinter, ungekuerzt ausgelagert: die vier Muster aus dem Netz, der veraltete Schluesselbund-Einwand fuer macOS, und die vier `claude auth status`-Laeufe, die den Token-Weg widerlegt haben
