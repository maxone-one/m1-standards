...
> Dieses Dokument ist die Begründung hinter den Standards zu Gates, Sicherheit
> und Betrieb. Es ersetzt sie nicht, es zeigt, **was sie abdecken und was
> nicht**.
>
> **Die Nummern im Text sind am 09.09.2026 auf den Neuschnitt nachgezogen.**
> Wo hier noch eine alte Nummer steht, loest sie die Umleitungstabelle in
> [README.md](README.md) auf.
...
|---|---|---|---|---|
| **Semgrep** | OSS + paid | ✅ self-host | XSS, SSRF, Injection, Path-Traversal | ✅ Standard 010 |
...
| Tool | Lizenz | EU-OK | Findet | Bei uns im Einsatz |
|---|---|---|---|---|
| **gitleaks** | OSS | ✅ self-host | Secrets in Code + Git-Historie | ✅ Standard 010 |
...
| Tool | Lizenz | EU-OK | Findet | Bei uns im Einsatz |
|---|---|---|---|---|
| **npm audit / pnpm audit** | built-in | ✅ lokal | CVEs in Dependencies | ✅ Standard 023-A |
...
| **Custom curl-Tests** | DIY | Anon-Key kann lesen was er soll | ✅ Standard 023-C |
...
| 4 | Pre-Launch (manuell) | Standard 023 komplett, inkl. Section J | 013 |
...
- **Coverage:** ✅ Standard 010 (semgrep), ✅ Standard 023 Section J Punkt 1
...
- **Coverage:** ✅ Standard 010 (semgrep), ✅ Standard 023 Section J Punkt 2
...
- **Coverage:** ✅ Standard 010 (semgrep), ✅ Standard 023 Section J Punkt 3
...
- **Coverage:** ✅ Standard 010 (gitleaks), ✅ Standard 023 Section F (Frontend-Bundle), ✅ Section J Punkt 4
...
- **Coverage:** ⚠️ teilweise via Standard 010 (Lockfile-Pflicht), Socket.dev empfohlen für Slopsquatting-Erkennung. **Standard 023 (CONCEPT.md)** erzwingt explizite Stack-Wahl + Reviewer-Prüfung, Hallucinations fallen beim manuellen Code-Re-Read auf. 🔴 **TODO:** automatisierter Library-Existenz-Check in CI.
...
- **Coverage:** ✅ Standard 023 Section B (curl User-A vs User-B), ✅ Section J Punkt 7, ✅ Standard 023 (Pen-Test-Light: defensive Außensicht, exposed Files / Admin-Routen / Status-Endpunkte / Header-Hygiene). Echte BOLA mit zwei Test-Usern bleibt manuell in 013-C / Section J.

#### B2: Vertical Privilege Escalation
- **OWASP:** A01
- **Beschreibung:** Normaler User wird Admin durch Manipulation von Header/Token/Request
- **Coverage:** ✅ Standard 023 Section B
...
- **Coverage:** ✅ Standard 010 (semgrep p/owasp-top-ten)

#### B5: Insecure Design
- **OWASP:** A04
- **Beschreibung:** Fehlende Threat-Modeling, fehlende Sicherheitsanforderungen vor Bauphase
- **Coverage:** 🔄 Standard 023 geplant (CONCEPT.md / Gate 1)
...
- **Coverage:** ✅ Standard 023 Section C (RLS-Pflicht, Anon-Key-Test, Supabase-Linter manuell), 🔄 Continue CLI + Supabase MCP als daily Cron empfohlen (siehe Tools-Tabelle)
- **Coverage Container-Layer:** ✅ Standard 003 (Container-Misconfig-Audit), 7 Pflicht-Klassen pro `docker-compose.yml`: privileged, inline-secrets, `:latest`-Pull, mem_limit, restart, docker.sock, env_file aus `/opt/secrets/`. SSH-first mit Local-Fallback, schließt den dokumentierten 002+004-Compose-Blindspot.

#### B7: Vulnerable & Outdated Components
- **OWASP:** A06
- **Coverage:** ✅ Standard 023 Section A (`npm audit --production` ohne Critical/High), Dependabot empfohlen
...
- **Coverage:** ✅ Standard 023 Section G (Webhook-Signatur-Prüfung), ✅ npm lockfile Pflicht

#### B10: Security Logging & Monitoring Failures
- **OWASP:** A09
- **Beschreibung:** Vorfälle werden nicht erkannt, weil Logging fehlt oder PII enthält
- **Coverage:** ✅ Standard 023 Section H, I (Logging + Monitoring Pflicht)
...
- **Coverage:** ✅ Standard 023 Section D + ✅ Standard 012 (audit.mjs HTML-Pattern-Scan auf GA/GTM/FB-Pixel/Hotjar/Mixpanel/Segment/Amplitude/Intercom/CrazyEgg/LinkedIn/TikTok/YouTube/Vimeo/Maps; Webbkoll für JS-injizierte Tracker manuell)

#### C2: Google Fonts via CDN
- **Rechtsgrundlage:** **LG München I, Az. 3 O 17493/20**, Schadensersatz an betroffene IP
- **Coverage:** ✅ Standard 023 Section D + ✅ Standard 012 (audit.mjs HTML-Pattern-Scan auf `fonts.googleapis.com` / `fonts.gstatic.com`, am 2026-04-27 hat das Audit repivot + snapflow live als Verstoss geflaggt)
...
- **Coverage:** Standard 023 Section D + Standard 009 (data_processors-Registry mit AVV-/DPA-Status, Nachweis-Ort und reviewed_at)

#### C5: Daten ausserhalb EU
- **Rechtsgrundlage:** DSGVO Kap. V
- **Coverage:** ✅ Standard 023 Section D, ✅ "Germany First"-Prinzip in CLAUDE.md

#### C6: Externe Embeds ohne 2-Click
- **Beispiel:** YouTube, Vimeo, Maps mit Personenbezug
- **Coverage:** ✅ Standard 023 Section D
...
- **Coverage:** ✅ Standard 023 Section C (RLS) + Section J Punkt 6 (Unauth-Routes-Liste) + ✅ Standard 023 (Pen-Test-Light: prüft `.env`/`.git/`/`backup.sql` exposed, am 2026-04-28 lief der Audit live über alle 8 Domains, 0 Critical-Treffer). Vollständiger Endpunkt-Scan auf personenbezogene Felder bleibt manuell.
...
- **Coverage:** ✅ Standard 011 (3-Direktiven-Härtung + `<user_message>`-Wrapping + Pflicht-Test-Suite mit ≥10 Payloads)
...
- **Coverage:** ✅ Standard 011 (DOMPurify-Pflicht + Tool-Use-Schema mit `enum`-Whitelists statt Freitext-Aktionen)
...
- **Coverage:** ✅ Standard 011 (Direktive 2: explizites Verbot der Prompt-Preisgabe inkl. ROT13/Base64/Translation-Bypass)
...
- **Coverage:** ✅ Standard 023 Section E (Test/Prod-Trennung) + ✅ Standard 011 (Approval-Queue über `ops_tasks` für Schreib-Tools)
...
- **Coverage:** ⚠️ teilweise via Standard 011 (Indirect-Input-Wrapping); 🔴 **TODO:** dedizierte Memory-Validierung vor jedem Restore in VECTOR
...
- **Coverage:** ⚠️ teilweise via Standard 011 (Approval-Queue + Whitelists); 🔴 **TODO:** Output-Sanity-Checks zwischen Agent-Hops
...
- **Coverage:** ✅ Standard 023 Section J Punkt 8 (Plattform-Lock-in-Check), ✅ Standard 024 (Stack-Whitelist + Audit auf Marker/Lockfile)
...
- **Coverage:** ✅ Standard 023 Section A (npm audit), Dependabot empfohlen
...
- **Coverage:** ✅ Standard 023 Section F (Source-Maps in Prod deaktiviert/privat) + ✅ Standard 012 (Live-Asset-Scan auf `sourceMappingURL=` Direktive), ⚠️ npm-Package-Inhalt nicht systematisch geprüft
...
- **Coverage:** ✅ Standard 007 (audit.mjs `dns.resolve4` gegen `KNOWN_SERVER_IPS`-Whitelist; fremde IPs werden mit konkreter IP-Liste gewarnt)

#### F2: Bundle-Drift (alte URLs / Domains)
- **Beispiel:** repivot lädt `panel.maxone.studio` obwohl auf `.one` migriert
- **Coverage:** ✅ Standard 012 (audit.mjs zieht bis zu 8 Live-Assets pro Domain und scannt auf `*.maxone.studio`-Reste, Plattform-Watermarks, Dev-Hosts, Service-Role-Keys; am 2026-04-27 hat das Audit bei repivot genau diesen Drift live nachgewiesen)
...
- **Coverage:** ✅ Standard 007 (audit.mjs `tls.connect` zieht Cert, prüft Restlaufzeit (>14d OK / 7-14d WARN / <7d FAIL), Issuer (Let's Encrypt) und Subject/SAN-Match)

#### F4: Source-Maps in Production
- **Beschreibung:** Source-Maps öffentlich → Reverse-Engineering trivial
- **Coverage:** ✅ Standard 023 Section F + ✅ Standard 012 (Live-Asset-Scan)
...
- **Coverage:** ✅ Standard 031 (Mail-Architektur, Pre-Flight-Pflicht, Anti-Pattern-Scan für `uploadUrl.split("{")` / `Basic healthcheck:invalid` / `/.well-known/jmap`-Request / Public-URL aus Edge-Function / fehlender DB-Status `rejected_unauthenticated_domain`). Quelle: [`maxone.one/briefings/ZENTINEL-STALWART-BIBEL.md`](https://github.com/maxone-one/maxone.one/blob/main/briefings/ZENTINEL-STALWART-BIBEL.md) (20 Regeln, lebendiges Dokument).
...
- **Coverage:** ✅ Standard 023 Section A (Black-Box-Anteil Pflicht-Schätzung,
  > 20 % triggert zusätzlichen Review-Pass), ✅ Standard 023 (CONCEPT.md
...
1. **Semgrep CE**, SAST (Standard 010)
2. **gitleaks**, Secret-Scan (Standard 010)
...
### Vor jedem Go-Live (Standard 023-Lauf)
...
3. **Pen-Test-Frequenz**, einmalig vor Launch (aktuell), jährlich, oder
   bei jedem Major-Release? Standard 023 (Pen-Test-Light) deckt automatisiert
...
5. **Security-Disclosure-Policy**, eigener `/security.txt` mit Kontakt und
   Response-Zeit pro Domain? Würde unter Standard 024 fallen.

6. **DSGVO-Folgenabschätzung (Art. 35)**, ab wann formal? Standard 023
...
| 21 | Stalwart-Orphan-Vorfall 2026-03-24 (CLAUDE.md), `docker run` ohne `restart:` → Brevo-Key-Exposure, Mail-Downtime | B6 (Standard 003 Begründung) |
