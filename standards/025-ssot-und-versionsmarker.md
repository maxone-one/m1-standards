# 025: SSoT & Version-Marker (Version-Marker · Cron-Dedup · Kein Hardcode)
...
**Kein automatisches Neuladen.** Eine Seite, die sich unter den Haenden des Nutzers erneuert, zerstoert halb ausgefuellte Eingaben. Der Nutzer entscheidet, die Seite informiert nur. Das ist zugleich die ehrliche Variante des Nudgings (Standard 019): sichtbarer Wert-Tausch, volle Wahlfreiheit.
...
| Rechtliche Texte / Impressum | Zentrale API (→ Standard 017) |
| Secrets & API-Keys | `/opt/secrets/` Store (→ Standard 009) |
| Build-IDs | ENV-Injection zur Build-Zeit (→ A oben) |
| Farbpaletten / Tokens | `@theme`-Block in der globalen CSS-Datei (Tailwind v4) / CSS-Custom-Properties (→ Standard 024 C) |
