# 004: Zentrale Infrastruktur: Bauen statt melden
...
| Mail-Gateway | https://mail.maxone.one | EINZIGER Engpass für allen ausgehenden Mailverkehr (tx + outreach), fail-closed + Consent + append-only Audit. Einziger Halter der Provider-Keys. Spec: Standard 031-A. |
...
Jede Erweiterung wird dokumentiert und sofort auf bestehende Projekte angewendet wenn sie profitieren können. Broadcast via Standard 026-C (Cross-Project).
...
- **Direkter Mail-Provider-Aufruf aus IRGENDEINEM Repo** (Brevo, SMTP, SendGrid, Mailgun, SES …). Aller Mailversand läuft über das Gateway `mail.maxone.one`, Provider-Keys nur dort. Spec: Standard 031-A.
