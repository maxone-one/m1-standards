# 005: Routine-Platform: keine Cron-Logik in IDE/Claude-Sitzungen
...
1. **Stille Aussetzer**, User merkt erst nach Wochen, dass der
   tägliche Drift-Scan nicht mehr läuft. Standards 012 (Bundle-Drift),
   007 (Cert/DNS), 031 (Mail-Architektur) verlieren ihre Wirkung,
...
- **`brevo-bounce-watchdog.timer`** (systemd auf maxone-prod), fängt
  jede stillschweigend abgewiesene Sender-Domain (Standard 031 /
  Bibel Regel 12+20).
...
| Cert-Renewal | Traefik built-in (DNS-01-Loop, Standard 009) | bereits idempotent + idle-fähig |
...
- 026 Re-Review-Reminder, der ursprüngliche Anwendungsfall (Cron
  alle 180 Tage)
- VULN-CATALOG F (Drift-Klassen), alle F-Punkte (DNS, Bundle, Cert,
  Mail) brauchen Detection-Routinen, deren Plattform dieser
  Standard regelt
